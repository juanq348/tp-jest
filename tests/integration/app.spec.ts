...
describe('Todo API', () => {
  ...
  it("GET /todos/stats devuelve los los totales", async () => {
    await request(app).post("/todos").send({ title: "Tarea A" });
    const b = await request(app).post("/todos").send({ title: "Tarea B" });
    await request(app).post("/todos").send({ title: "Tarea C" });
